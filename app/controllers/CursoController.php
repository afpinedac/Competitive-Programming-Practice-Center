                ...
                if ($tab == 'evaluacion') {
                    ...
                    if ($ejercicio) { #si existe el ejercicio al que quiero acceder   
                        return View::make('curso.ejercicio.ejercicio2')
                                        ...
                                        //  ->with('logro', $usuario->get_logro_ejercicio(Auth::user()->id, $curso->id))
    ...
    public function getMonitorear($curso, $opcion = 'talleres', $value1 = null, $method = null, $value2 = null) {
        ...
        if ($curso->profesor_id == Auth::user()->id) {     #si el logueado es el profesor que creo el curso
            ...
            if ($opcion == "talleres") {
                ...
                if (Input::has('sortby') && in_array(Input::get('sortby'), array('nombres', 'tiempo_logueado', 'ejercicios_resueltos', 'puntos', 'ultimo_acceso'))) {
                    $sortby = Input::get('sortby');
                }

                $estudiantes = $curso->get_estudiantes_sort('estudiantes', $sortby, $value1, $curso->id);
